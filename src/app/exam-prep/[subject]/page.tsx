...
export default async function ExamSubjectPage({ params }: { params: Promise<{ subject: string }> }) {
        ...
        <div className="h-[100dvh] bg-gradient-to-br from-indigo-900 via-purple-900 to-slate-900 font-[family-name:var(--font-geist-sans)] overflow-hidden flex flex-col">
            <div className="max-w-md lg:max-w-2xl mx-auto w-full p-4 flex-1 flex flex-col h-full">
                <header className="flex justify-between items-center mb-6 shrink-0">
                    <a href="/exam-prep" className="bg-white/10 hover:bg-white/20 px-4 py-2 rounded-xl text-sm font-bold transition-colors">
                        ← Mundos
                    </a>
                ...
                <div className="flex-1 min-h-0">
                    <ExamEngine profile={profile} subject={subjectWorld} />
                </div>
            </div>
        </div>
    );
